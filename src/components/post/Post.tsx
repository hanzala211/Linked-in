  ...
  return <div className="bg-white rounded-lg overflow-hidden border border-gray-200">
    <div className="px-4 py-3 flex items-start gap-3">
      ...
      <div className="flex-1">
        ...
        <p className="text-[#666] text-[13px] text-ellipsis max-w-[30rem] line-clamp-1">{item.postBy?.bio}</p>
    ...
    <div className="mx-5 py-2 flex gap-4 justify-between">
      ...
      <button className="flex gap-2 items-center text-[15px] justify-center hover:bg-slate-100 w-full max-w-lg rounded-lg px-4 py-2" onClick={() => setIsCommentExpanded(true)}><BiMessageRoundedMinus /> Comment</button>
    ...
    <div className={`h-full transition-all duration-300 ${isCommentExpanded ? "max-h-[99rem] opacity-100 p-4" : "max-h-[0rem] opacity-0 overflow-hidden"}`}>
      <div className="flex gap-4 items-center relative">
        ...
        <button className={`text-white bg-[#0A66C2] px-2 text-[13px] py-1 rounded-full ${commentValue.length > 0 ? "opacity-100" : "opacity-0 pointer-events-none"} transition-all duration-100 hover:bg-opacity-70 absolute right-12`}>Comment</button>
      ...
      <button className="flex gap-2 mt-5 items-center text-[14px]"><span className="bg-[#EAE6DF] p-2 rounded-full"><TbArrowsDiagonal2 className="text-[17px]" /></span> <span className="hover:bg-[#EAE6DF] p-1 rounded-md transition-all duration-200">Load more comments</span></button>
    </div>
  </div >
}