...
export const ProfilePost: React.FC<ProfliePostProps> = ({ isCurrentProfile, item }) => {
  ...
  return <div className="bg-white border-[1px] w-fit mt-3 border-gray-200 p-3 rounded-lg">
    <Link to="#" className="flex gap-3">
      ...
      <div className="space-y-0.5">
        <h1 className="text-[17px] hover:text-[#0A66C2] transition-all duration-200 hover:underline">{isCurrentProfile ? userData?.firstName : selectedProfile?.firstName} {isCurrentProfile ? userData?.lastName : selectedProfile?.firstName}</h1>
    ...
    <Link to="#">
      <img src={item.imageUrls.length > 0 ? item.imageUrls[0] : ""} alt={`${item.postBy} Post`} className="w-96 mt-1 h-96 object-contain" />
    </Link>
